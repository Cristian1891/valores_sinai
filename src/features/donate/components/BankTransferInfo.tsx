...
// Mejora menor: aria-label más descriptivos en CopyButton.
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-3xl">
        ...
        {/* Aviso cuenta USD */}
        {/* {activeAccount === 'usd' && (
          <div className="mb-4 rounded-xl border border-brand-blue/30 bg-brand-blue/5 p-4">
            <p className="text-xs leading-5 text-dark-soft dark:text-gray-mid">
              💵{' '}
              {t(
                'bank.usdNote',
                'Esta cuenta recibe transferencias en dólares desde cualquier banco o billetera. Al ingresar dólares, el banco destino aparece como Banco Industrial (BIND).',
              )}
            </p>
          </div>
        )} */}
        ...
        {BANK_ACCOUNTS.filter((a) => a.id === activeAccount).map((account) => (
          ...
          >
            {account.fields.map((field) => (
              <div key={field.labelKey} className="flex items-center justify-between px-5 py-3.5">
                <div>
                ...
                <CopyButton text={field.value} label={t(field.labelKey)} />
              </div>
            ))}
          </div>
        ))}
