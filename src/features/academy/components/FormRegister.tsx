...
const DESTINATION_EMAIL = 'cristianovejero1891@gmail.com';
...
export const FormRegister: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    // Etiqueta legible del área seleccionada
    ...
    try {
      const response = await fetch(
        `https://formsubmit.co/ajax/${encodeURIComponent(DESTINATION_EMAIL)}`,
        {
          ...
          body: JSON.stringify({
            ...
            _subject:  `[Academia Sinaí] Nueva consulta de interés — ${data.nombre}`,
            _replyto:  data.email,
            _captcha:  'false',
            _template: 'table',
          }),
      ...
      console.error('[FormRegister - FormSubmit] Error al enviar:', error);
          ...
          <div className="overflow-hidden rounded-3xl bg-linear-to-br from-dark to-dark-soft p-8 sm:p-10 shadow-2xl ring-1 ring-white/10">
            <div className="flex flex-col items-center gap-5 py-10 text-center">
              ...
              <h2 className="text-2xl font-bold text-white">
                {t('form.successTitle', '¡Consulta recibida!')}
              </h2>
              <p className="max-w-sm text-sm leading-7 text-white/80">
                {t(
                  'form.successText',
                  'Te avisamos en cuanto haya novedades sobre los cursos.¡Gracias por tu interés en Academia Valores Sinaí!'
                )}
              </p>
            </div>
          </div>
        ...
        <div className="overflow-hidden rounded-3xl bg-linear-to-br from-dark to-dark-soft p-8 sm:p-10 shadow-2xl ring-1 ring-white/10">
          ...
          {submitState === 'error' && (
            <div
              role="alert"
              aria-live="assertive"
              className="mb-6 flex flex-col items-start gap-4 rounded-2xl border border-red-800/30 bg-red-900/10 p-5"
            >
              <p className="font-sans text-sm font-bold text-white">
                ⚠️ {t('form.errorTitle', 'Algo salió mal')}
              </p>
              <p className="font-sans text-xs leading-5 text-white/70">
                {t(
                  'form.errorText',
              ...
              <button
                type="button"
                onClick={handleRetry}
                className="rounded-xl border border-white/10 px-4 py-2 font-sans text-sm font-semibold text-white transition-colors hover:bg-white/10"
              >
          ...
          <div className="flex flex-col gap-5">
            ...
            <Field
              id="telefono"
              label={t('form.labelTelefono', 'Teléfono')}
